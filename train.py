...
class Trainer:
    def __init__(self, config, run):
        ...
        embedding_matrix, output_embedding_matrix = vectors.get_vectors(
    ...
    def train(self):
        ...
        while self.epoch < self.config.epochs:
            ...
            if self.config.optimizer == "adam":
                ...
                raise ValueError(
                    f'"{self.config.optimizer}" is an invalid optimizer name!'
                )
            ...
            for (
                ...
                self._log_step("train", train_metrics)
                self._validate()
    ...
    def _validate(self):
        ...
        with torch.no_grad():
            ...
            self._log_step("val", val_metrics)
            self._log_summary("val")

            if self.config.checkpoint_metric is not None:
                if (
                    self.best_performance is None
                    or val_metrics[self.config.checkpoint_metric]
                    > self.best_performance
                ):
                    self.best_performance = val_metrics[self.config.checkpoint_metric]
    ...
    def _test(self):
        if hasattr(self.data, "test"):
            ...
            with torch.no_grad():
                ...
                self._log_step("test", test_metrics)
    ...
    def _log_step(
        ...
        **kwargs,
        ...
        self.run_history[run_type].append(metrics)

    def _log_summary(self, run_type):
        metrics_df = pd.DataFrame(self.run_history[run_type])
        ...
        for i, batch in enumerate(dataloader):
            ...
            if config.dynamic_halting_loss_weight:
                if "input_n_updates" in extra_output:
                    input_halting_loss = (
                        extra_output["input_n_updates"]
                        + extra_output["input_remainders"]
                    )
                    ...
                    loss = (
                        token_loss
                        + config.dynamic_halting_loss_weight * input_halting_loss
                    )

                if "output_n_updates" in extra_output:
                    mask = batch_output_ids_padding_mask[:, 1:]
                    output_halting_loss = (
                        extra_output["output_n_updates"]
                        + extra_output["output_remainders"]
                    )
    ...
    def _compute_metrics(
        ...
        # TODO: This is some hacky crap, but need a way to not have this
        # run for non-babi stuff.
        if len(set(task_ids)) > 1:
            solved_tasks = 0
            for task in set(task_ids):
                correct_sents = np.array(correct_sents)
                metrics[f"accuracy_task_{task}"] = correct_sents[
                    task_ids == task
                ].mean()
